...
class App extends Component<any, any> {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <button onClick={() => store.dispatch({ type: 'show' })}>
          显示列表
        </button>
...
// applyMiddleware 和 thunkMiddleware 使redux支持异步action
const store = createStore(
  totalReducers,
  initStateData,
  applyMiddleware(thunkMiddleware)
...
class HomeUi extends Component {
  ...
  componentDidMount() {
    // 通过使用this.props.location可以获取到路由的相关信息
    console.log('homeui组件从新挂载了', this.props.location);
  }
  render() {
