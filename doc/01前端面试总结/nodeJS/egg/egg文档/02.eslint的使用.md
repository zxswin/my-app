...
- 安装 airbnb 公司开源的 eslint 规则规范
  > npm i -D eslint-config-airbnb

## vscode html 等模板文件的格式

> 安装 Nunjucks Template Formatter 插件

## vscode 编辑器中使用 ESLint 的要点

```bash
1.必须先在项目的本地或者全局安装ESLint的库
2.在vscode 编辑器中下载安装 ESLint (编辑器中不启用ESLint貌似配置不生效)
3.vscode 启用了ESLint后 .eslintrc.js中的配置就立即生效了。
4.项目中安装的本地ESLint库是用于在代码编译过程中对代码进行格式检查及转换
5.vscode中的ESLint则是在代码编辑过程中的代码格式校验
6.本地ESLint库和vscode中的ESLint可共享同一份配置文件
7.可以在首选项-eslint的配置中启动保存时候自动修复错误(== 或 != 问题貌似必须手动修复)
8.ESlint：是用来统一JavaScript代码风格的工具，不包含css、html等。
```

## 使用 ESLint + Prettier 来统一我们的前端代码风格
...
```bash
1.Prettier支持对js,ts,css,less,json等文件进行格式化
2.可以格式js代码和css代码
3.可以实现粘贴代码自动格式化
4.可以实现保存代码自动格式化
5.可实现对当前行进行格式化
6.自动格式化框架键 alt + shift + f
```

- VScode settings.json 文件中配置(用于全局配置)
...
```js
{
  ...
  "editor.tabSize": 2,
  "editor.formatOnPaste": true, // 是否自动格式粘贴的内容
  "editor.formatOnSave": true, // 是否保存时自动格式化文件
  "editor.formatOnType": true, // 键入一行后自动自动格式化
  "typescript.updateImportsOnFileMove.enabled": "always",
  "files.autoSave": "off",
  "prettier.semi": false, // 每一行结束的时候是否使用分号结尾
  "prettier.singleQuote": true // 使用单引号
}

```

- 在项目根目录添加配置文件.prettierrc(独立配置)
...
/** 优先级高于全局配置  */
{
  ...
  // 指定每个缩进级别的空格数 默认2
  "tabWidth": 2,
  // 用制表格而不是空格缩进行 默认是false
  "useTabs": false,
  // 在语句末尾打印分号 默认是true
  "semi": true,
  // 使用单引号而不是双引号  默认 false
  "singleQuote": true,
  // 多行时尽可能打印尾随逗号。（例如，一个单行数组永远不会得到尾随逗号。） 默认"none"
  "trailingComma": "all",
  // 打印对象文本中括号之间的空格。默认 true
  "bracketSpacing": true,
  // 在唯一箭头函数参数周围包含括号。 默认值 "avoid"
  "arrowParens": "avoid",
  // 只格式化文件的一段。
  "rangeStart": 0,
  "rangeEnd": Infinity,
  // 指定要使用的分析器。 默认值 None 即 babylon
  "parser":"typescript",
  "parser": require("./my-parser"),
  // 指定用于推断要使用哪个分析器的文件名。 默认是None
  "filepath":"None",
  // 在文件顶部包含一个称为pragma的特殊注释 默认为false
  "requirePragma": false,
  // 文件顶部插入一个特殊的@format标记，指定文件已使用prettier格式化。 默认为false
  "insertPragma": false,
  // 散文包装 貌似和git提交有关 默认值 "preserve"
  "proseWrap": "preserve"
  // HTML空白敏感度 默认值 "css"
  "htmlWhitespaceSensitivity":"css"
  // 行尾(当人们在来自不同操作系统的项目上协作时，很容易在中央Git存储库中以混合行结尾) 默认为"auto"
...
- vscode 中 解决格式化后将单引号变双引号

```js
/** 首选项设置 - 搜索格式化 - 找到settings.json文件
