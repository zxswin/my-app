...
```html
<!-- 
  <transition> 组件上的 duration prop 显式指定过渡持续时间 (以毫秒计)：
-->
...
<!-- 
  ...
  JavaScript 过渡的时候，在 enter 和 leave 钩子中必须使用 done 进行回调。否则，它们将被同步调用，过渡会立即完成。添加 :css="false" 也会让 Vue 会跳过 CSS 的检测，除了性能略高之外，这也可以避免过渡过程中受到 CSS 规则的意外影响。
 -->
<transition
...
## 使用 GreenSock 的 JavaScript 过渡的例子
