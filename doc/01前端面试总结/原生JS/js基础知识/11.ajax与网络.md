...
# HTTPS协议的传输内容都被SSL/TLS加密， 且运行在SSL/TLS上，SSL/TLS运行在TPC连接上，所以数据传输是安全的。
```
...
xhr.open('GET', 'http://localhost/network?param1=1', true); // 发送设置参数
 ...
 * CSRF英文全称是 Cross-site request forgery,所以又称为“跨站请求伪造”
 *
 * 什么是CSRF 攻击
...
```ts
/**
 * ajax受同源策略限制不管是GET请求还是POST请求(待确认)
 *
 * 1.同源策略即：
