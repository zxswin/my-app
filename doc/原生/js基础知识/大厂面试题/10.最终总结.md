 ...
 *
 * 其中Object包含的数据类型
 * Data、Function、Array 、Set、Map、WeakSet,WeakMap
 */
```
...
```ts
/**
 * 假设在全局中定义了变量c,函数a,函数a里面还定义了函数b,并且变量b会在a里面执行,在全局中执行a函数;
 *
 ...
 * Cache-Control和Expires的区别在于Cache-Control使用相对时间，Expires使用的是基于服务器 端的绝对时间，因为存在时差问题，
